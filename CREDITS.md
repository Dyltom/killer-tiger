...
synthesis could not convincingly fake, so the scream cue and the village murmur
play recordings when they are present — see `src/engine/samples.ts` for the
reasoning. The synthesised versions are still in `audio.ts` and still run when
the samples are not there, which is a supported way to run the game. `shout`
has no recorded source yet and is still synthesised.

**Unlike every other asset here, not all of this is CC0.** The pack that
carries the scream cue is CC BY 3.0, so attribution is a licence condition
rather than a courtesy: shipping this game means shipping the credit to Michel
Baradari below. The CC0 alternatives were two files, against eleven usable
takes here, which is the trade that was made. Nothing is CC BY-NC — everything
below is free for commercial use.
