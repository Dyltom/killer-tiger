...
synthesis could not convincingly fake, so the two voice cues and the village
murmur play CC0 recordings when they are present — see `src/engine/samples.ts`
for the reasoning, and the synthesised versions are still in `audio.ts` and
still run when they are not.

`scripts/fetch-voices.ts` fetches those samples from
[Freesound](https://freesound.org/), filtering to Creative Commons 0 at the API
so nothing carrying an attribution or non-commercial obligation can enter the
build. It normalises them, writes `public/assets/audio/voices/manifest.json`,
and generates the per-file credits below from the API's own licence metadata
rather than from anything typed here by hand. It refuses to write credits at
all if a non-CC0 licence comes back. The processed output is committed, so a
clean checkout needs neither the network nor an API token.

<!-- voices:begin — generated by scripts/fetch-voices.ts -->

*No voice samples fetched yet; all voices are currently synthesised.*
