...
The scream and the shout each ship in two versions, `-f` for the women of the
village and unsuffixed for the men, chosen by which body the villager is wearing
rather than by pitching one set of takes up. See `Samples.pick`.

**Unlike every other asset here, not all of this is CC0.** Two packs are CC BY
3.0 — so attribution is a licence condition rather than a courtesy, and shipping
this game means shipping the two credits below.

- Michel Baradari's, which carries all eleven male screams. The CC0 alternatives
  for that cue were two files against eleven, which is the trade that was made.
- congusbongus's *Female screams*. Four of the five entries in that pack's own
  `license.txt` are CC0 and only one is CC BY, and the four files it actually
  ships are very probably the CC0 ones — but the pack has been re-edited and
  there is no way left to prove which original each file came from, so all four
  are credited under the stricter of the two licences. The comment on the pinned
  entry in `scripts/fetch-voices.ts` records what was checked.

Everything else here is CC0 or public domain, and nothing is CC BY-NC or
share-alike: all of it is free for commercial use, and only those two entries
carry an obligation.
...
- **scream-f** — [High pitch scream sounds(2)](https://opengameart.org/content/high-pitch-scream-sounds2) by pauliuw — CC0
- **scream-f** — [Female screams](https://opengameart.org/content/female-screams) by congusbongus, from Freesound recordings by thanvannispen, tcrocker68, pushkin and Archeos — CC BY 3.0
- **shout-f** — [Female RPG Voice Starter Pack](https://opengameart.org/content/female-rpg-voice-starter-pack) by cicifyre — CC0
